...
class LeftSlidePanel extends StatefulWidget {
  ...
  const LeftSlidePanel({
    required this.body,required this.slidePanelWidth,required this.slidePanelHeight, required this.slideHandlerWidth,
        required this.appbarIsExist});
  @override
  _LeftSlidePanelState createState() => _LeftSlidePanelState();
}

class _LeftSlidePanelState extends State<LeftSlidePanel> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    super.initState();
    _animationController = AnimationController(vsync: this, duration: toggleDuration);
  }
  ...
  void _onDragUpdate(DragUpdateDetails details) {
    if (_canBeDragged) {
      double delta = (details.primaryDelta ??0)/ widget.slidePanelWidth;
